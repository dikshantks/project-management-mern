    ...
    useEffect(() => {
        if (!isAddTaskModalOpen || isRenderChange) {
            axios
                ...
                .then((res) => {
                    const project = res.data; // Assuming the whole project object is returned
                    setTitle(project.title);

                    // Group tasks by stage
                    // const columns = project.task.reduce((acc, task) => {
                    //     const columnKey = task.stage || "Requested"; // Handle tasks without a stage
                    //     acc[columnKey] = acc[columnKey] || {
                    //         name: columnKey,
                    //         items: [],
                    //     };
                    //     acc[columnKey].items.push(task);
                    //     return acc;
                    // }, {});
